...
class Image:
    ...
    def __init__(self, maze: MazeGenerator, texts: list[str],
                 cell_size: int, color_dict: dict[str, list[int]],
        ...
        self.win_width = max(self.img_width,
                             max([len(text) * 10 for text in texts]) + 40)
        ...
        self.img_ptr = self.mlx.mlx_new_image(
            self.mlx_ptr, self.img_width, self.img_height)
    ...
    def color_element(self, element: str,
    ...
    def color_rect(self, element: str, start_pixel: int,
                   rect_height: int, rect_width: int,
                   paint: bool = True) -> None:
    ...
    def color_cell(self, element: str, curr_cell: tuple[int, int],
                   paint: bool = True) -> None:

        x_curr, y_curr = curr_cell
        start = (y_curr * self.size_line + x_curr * 4) * self.cell_size
        start += (self.size_line + 4) * self.wall_width
        self.color_rect(element, start, self.maze_width,
                        self.maze_width, paint)

    def draw_grid(self) -> None:

        # Coloring the horizontal lines
        self.color_rect("maze_num_wall", 0,
                        self.wall_width, self.img_width, False)
        start = (self.cell_size - self.wall_width) * self.size_line
        for i in range(1, self.maze.height):
            self.color_rect("maze_num_wall", start, 2 * self.wall_width,
                            self.img_width, False)
            start += self.cell_size * self.size_line
        self.color_rect("maze_num_wall", start, self.wall_width,
                        self.img_width, False)
        # Coloring the vertical lines
        self.color_rect("maze_num_wall", 0, self.img_height,
                        self.wall_width, False)
        start = (self.cell_size - self.wall_width) * 4
        for i in range(1, self.maze.width):
            self.color_rect("maze_num_wall", start, self.img_height,
                            2 * self.wall_width, False)
    ...
    def color_wall(self, element: str, curr_cell: tuple[int, int],
        ...
        curr_cell_start = (y_curr * self.size_line +
                           x_curr * 4) * self.cell_size
        curr_cell_start += (self.size_line + 4) * self.wall_width
        next_cell_start = (y_next * self.size_line +
                           x_next * 4) * self.cell_size
        ...
        if y_curr == y_next and abs(x_curr - x_next) == 1:
            ...
            self.color_rect(element, start, 2 * self.wall_width,
                            self.maze_width)
            self.color_rect(element, start, 2 * self.wall_width,
                            self.maze_width)
            return (1)
    ...
    def algorithm_sequence(self) -> Generator[None, None, None]:
        ...
        for next_cell in cell_sequence:
            while not self.color_wall("background", curr_cell,
                                      next_cell, taken_cells):
                cell_hist.pop()
                ...
                curr_cell = next_cell
                yield
        background = set(self.pixel_dict["background"])
        maze_num_wall = set(self.pixel_dict["maze_num_wall"])
        self.pixel_dict["background"] = list(background)
        self.pixel_dict["maze_num_wall"] = list(maze_num_wall - background)
        ...
        if self.building_maze:
            ...
            if (self.color_dict["del_walls"] != [0, 0, 255]):
                ...
                if (now - self.last_time > self.burst_delay):
                    b, g, r = self.color_dict["del_walls"]
                    self.color_dict["del_walls"] = [0, 0, r + 5]
                    self.color_element("del_walls")
                    self.last_time = time.monotonic()
            else:
                self.color_dict["del_walls"] = self.color_dict["background"]
                self.color_element("del_walls")
                self.pixel_dict["background"] += self.pixel_dict["del_walls"]
                del self.pixel_dict["del_walls"]
                del self.color_dict["del_walls"]
                self.color_cell("entry", self.start)
                self.color_cell("exit", self.end)
                self.deleting_walls = False
        return

    # This function draws the moves player does
    def move(self) -> None:
    ...
    def on_key(self, keycode, param) -> None:
        ...
        if keycode == 110 and not (self.building_maze or self.deleting_walls):
            self.maze = MazeGenerator(
                self.maze.width, self.maze.height, self.maze.start,
            ...
            self.pixel_dict["del_walls"] = []
            self.color_dict["del_walls"] = [0, 0, 100]
            self.draw_grid()
        ...
        if (keycode in [65361, 65362, 65363, 65364] and not
                (self.building_maze or self.deleting_walls)):
            if self.curr_cell != self.start:
                self.color_cell("player_path", self.curr_cell, False)
            # Left arrow and is able to move left
            if (keycode == 65361 and not
                    self.maze.walls_config[self.curr_cell][3]):
                x_pos, y_pos = self.curr_cell
                self.curr_cell = (x_pos - 1, y_pos)
                self.move()
            # Up arrow and is able to move up
            elif (keycode == 65362 and not
                    self.maze.walls_config[self.curr_cell][0]):
                x_pos, y_pos = self.curr_cell
                self.curr_cell = (x_pos, y_pos - 1)
                self.move()
            # Right arrow and is able to move right
            elif (keycode == 65363 and not
                    self.maze.walls_config[self.curr_cell][1]):
                x_pos, y_pos = self.curr_cell
                self.curr_cell = (x_pos + 1, y_pos)
                self.move()
            # Down arrow and is able to move down
            elif (keycode == 65364 and not
                    self.maze.walls_config[self.curr_cell][2]):
                x_pos, y_pos = self.curr_cell
                self.curr_cell = (x_pos, y_pos + 1)
                self.move()
